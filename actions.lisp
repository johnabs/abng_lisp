(defun action1 (adjacency-matrix)
  ...
  (let*
      (
       ...
       (norm (cl-ana.tensor:tensor-map #'nan-div d2 (make-list nodes :initial-element (cl-ana.linear-algebra:euclidean-dot d2 (ones nodes)))))
...
(defun action2 (adjacency-matrix)
  ...
  (let*
      (
       ...
       (nodes (length adjacency-matrix))
       (deg (cl-ana.linear-algebra:euclidean-dot  (ones nodes) adjacency-matrix))
       (norm (cl-ana.tensor:tensor-map #'nan-div deg (make-list nodes :initial-element (cl-ana.linear-algebra:euclidean-dot deg (ones nodes)))))
       (m (make-array (list nodes nodes) :initial-element 0.0)))
    ;; Repeat norm vector to create matrix
    (dotimes (i nodes)
      (dotimes (j nodes)
...
(defun action5 (input)
  (let* ((n (length input))
         (a2 (list-to-array (tensor-to-list (cl-ana.linear-algebra:matrix-mult input input))))
         (adj (list-to-array input))
...
(defun action7 (adj-list )
  ...
  (let* ((n (length adj-list))
    ...
    (normalize-tensor (cl-ana.linear-algebra:lisp-2d-array->tensor result-matrix))))
