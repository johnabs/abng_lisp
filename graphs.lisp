...
(defun col-sums (matrix)
  ...
  (lla:mm (aops:permute '(1 0) matrix) (ones (array-dimension matrix 0))))

(defun row-sums (matrix)
  "Calculate sum of each row in matrix"
  (lla:mm matrix (ones (array-dimension matrix 0)))
  )
...
(defun ith-degree (adj-tensor vertex)
  ...
  (let ((n (first (array-dimensions adj-tensor))))
    (loop for i below n
          sum (aref adj-tensor vertex i))))
...
(defun neighbors (adj-tensor vertex)
  ...
  (let* ((n (first (array-dimensions adj-tensor)))
         (neighbors-list '()))
    (dotimes (i n)
      (when (= 1 (aref adj-tensor vertex i))
...
;; (defun vecsum (mat)  ( mat (ones (length mat))))
(defun degree (adjacency-matrix)
  ...
  (col-sums adjacency-matrix)
  )


(defun normalize-array (input)
  (aops:each (alexandria:rcurry #'/ (lla:asum input)) input))
  ...
  (let* ((n (array-dimension matrix 0))
    ...
    (loop for i from 0 below n do
      (loop for j from 0 below n do
        (incf (aref col-sums j) (aref matrix i j))))
...
(defun get-neighbors (graph vertex)
  (let ((n (get-vertex-count graph))
        (neighbors nil))
    (dotimes (i n)
      (when (not (zerop (aref graph vertex i)))
        (push i neighbors)))
    (nreverse neighbors)))
...
(defun matrix-to-adjacency-list (matrix)
  "Convert an adjacency matrix (2D array) to an adjacency list representation.
   Each element in the result is a cons of (vertex . neighbors) where neighbors is
   a list of vertices that the vertex is connected to."
  (let ((rows (array-dimension matrix 0))
        (cols (array-dimension matrix 1)))
    (loop for vertex from 0 below rows
          collect (cons vertex
                        (loop for neighbor from 0 below cols
                              when (= (aref matrix vertex neighbor) 1)
                                collect neighbor)))))
...
(defun brandes (input &optional (directed nil))
  ...
  (let (
        ...
        (vertices (alexandria:iota (array-dimension input 0)))
